...
function count(inputs){
  ...
  allItems.forEach(function(allitem){
    if(flag1==0&&allitem.barcode=='ITEM000001'){
      outputs.push({barcode:allitem.barcode,name:allitem.name,unit:allitem.unit,price:allitem.price,count:count[0]});
      flag1=1;
    }
    if(flag2==0&&allitem.barcode=='ITEM000003'){
      outputs.push({barcode:allitem.barcode,name:allitem.name,unit:allitem.unit,price:allitem.price,count:count[1]});
      flag2=1;
    }
    if(flag3==0&&allitem.barcode=='ITEM000005'){
      outputs.push({barcode:allitem.barcode,name:allitem.name,unit:allitem.unit,price:allitem.price,count:count[2]});
      flag3=1;
    }
...
var promotion=[];
  ...
  outputs.forEach(function(output){
    if(output.barcode=='ITEM000001'/*promotion[0]["barcodes"][0][0]||output.barcode==promotion[0]["barcodes"][1][0]*/)
    {
      if((output.count)%2==0)
    ...
    else if(output.barcode=='ITEM000005'/*output.barcode==promotion[0]["barcodes"][0][1]||output.barcode==promotion[0]["barcodes"][1][1]*/)
    {
